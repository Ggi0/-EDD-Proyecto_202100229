...
AVL::AVL(const AVL& other) : raiz(nullptr) {
...
void AVL::liberarMemoria(NodoAVL* nodo) {
    if (nodo != nullptr) {
        liberarMemoria(nodo->getIzq());
        liberarMemoria(nodo->getDrcha());
        delete nodo;
    }
}

// Método auxiliar para copiar nodos
NodoAVL* AVL::copiarNodo(NodoAVL* nodo) {
    if (nodo == nullptr) return nullptr;
    
    NodoAVL* nuevoNodo = new NodoAVL(nodo->getData());
    nuevoNodo->setAltura(nodo->getAltura());
    
    nuevoNodo->setIzq(copiarNodo(nodo->getIzq()));
    nuevoNodo->setDrcha(copiarNodo(nodo->getDrcha()));
