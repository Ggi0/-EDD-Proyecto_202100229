...
listaSimpleEnlazada::listaSimpleEnlazada(const listaSimpleEnlazada& other) {
    primero = nullptr;
    ultimo = nullptr;
    
    // Copiar todos los nodos
    Nodo* temp = other.primero;
    while (temp != nullptr) {
        append(temp->getData());
        temp = temp->getSig();
    }
...
listaSimpleEnlazada& listaSimpleEnlazada::operator=(const listaSimpleEnlazada& other) {
    if (this != &other) {
...
int listaSimpleEnlazada::findIndex(int valor) const{
    Nodo *temp = primero;
    int indice = 0;
